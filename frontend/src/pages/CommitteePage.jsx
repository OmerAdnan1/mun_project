  ...
  useEffect(() => {
    const fetchCommitteeData = async () => {
      try {
        ...
        if (committeeData) {
          const [delegatesData, documentsData, blocksData] = await Promise.all([
            committeesAPI.getCommitteeDelegates(id),
            committeesAPI.getCommitteeDocuments(id),
            committeesAPI.getCommitteeBlocks(id),
          ])
            ...
            <div>
              ...
              {blocks.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {blocks.map((block) => (
                    <li key={block.block_id} className="p-4 bg-white rounded shadow">
